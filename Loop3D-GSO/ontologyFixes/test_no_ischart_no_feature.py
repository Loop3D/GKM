#!/usr/bin/env python3
"""Test full GSO ontology excluding Ischart and Feature modules.
...
GSO-Geologic_Time_Ischart is excluded due to known inconsistency
with timeFinishedBy/timeIncludes property chain.
"""
...
def get_ontology_files(base_path):
    ...
    # Exclude Ischart (known inconsistency) and Feature (not imported by Master)
    excluded = []
    result = []
    for f in files:
        basename = os.path.basename(f).lower()
        if 'ischart' in basename or 'ischart' in basename.lower():
            excluded.append(f)
        elif basename == 'gso-feature.ttl':
            excluded.append(f)
        else:
            result.append(f)
