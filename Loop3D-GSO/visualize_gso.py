...
class hierarchy with property details panel. Default root is Solid_Geologic_Material but any
class URI can be used as root in the interactive viewer.
...
class GSOVisualizer:
    ...
    def load_ontology(self, base_path):
        ...
        module_files = [
            ...
            "Modules/GSO-Geologic_Time_Scales.ttl",
            "Modules/GSO-Geologic_Time_Scales-GTS.ttl",
...
''')
  ...
  <g transform="translate(20, {legend_y})">
    ...
    <text x="25" y="22" class="legend-text">Root Class</text>
...
def main():
    """Main entry point"""
    # Get the directory of this script (repo root, where ontology files live)
    ...
    interactive_html = os.path.join(base_dir, "visualizeGSO.html")
    viz.generate_interactive_html(interactive_html, root_uri)

    # Generate a detailed SVG view
    detailed_svg = os.path.join(base_dir, "visualizeGSO_hierarchy.svg")
